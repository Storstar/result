...
def build_scenario_concept(
    ...
    return ScenarioConcept(
        ...
        character_description=f"A believable on-camera creator representing {client_brief.target_audience}.",
        environment_description="Native short-form creator environment that matches the audience and the product use case.",
        hook_text=opening_segment,
        hook_type="pain",
        post_hook_voiceover=voiceover_plan.segments[1].suggested_line if len(voiceover_plan.segments) > 1 else voiceover_plan.full_voiceover_draft,
